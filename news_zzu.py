...
# 插入spider结果表
insert_result = '''
INSERT INTO t_spider_result VALUES (NULL, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NULL)
'''
                ...
                if not judge:
                    ...
                    if search_refresh:
                        ...
                        if judge_identifier:
                            ...
                            # html转pdf
                            pdfkit.from_url(refresh_url, pdf_file, configuration=confg)
                            ...
                            f_data.seek(0, 0)
                            f_data.write(json_data)
                            print('该新闻《{}》网页不存在， 以‘404 not found’为网页内容存入数据库。'.format(news_title))
...
def main():
    with open('dict_data.txt', 'r+') as f_data:
        ...
        # 郑大新闻网所有的栏目链接
        all_urls = all_urls_list(f_data)

        # all_urls.remove('http://www16.zzu.edu.cn/msgs/vmsgisapi.dll/vmsglist?mtype=x&lan=202')
        # all_urls = ['http://www16.zzu.edu.cn/msgs/vmsgisapi.dll/vmsglist?mtype=m&lan=105']

        for url in all_urls:
            url_list = get_url_list(url, f_data)
            get_url_info(url_list, f_data)
